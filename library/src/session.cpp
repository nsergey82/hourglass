...
int Session_create_f(Session_t **handle, SessionHandlerDispatcherPtr dispatcher,
                     Session_cb_t *fromUser) {
    *handle = HgHandles::release(
            make_myshared<HG::SessionImpl>(dispatcher, fromUser));
    return 0;
}
...
int Session_copy(const Session_t *src, Session_t **dest) {
    *dest = reinterpret_cast<Session_t *>(HgHandles::clone(
            const_cast<Session_t *>(src)));
    return 0;
}
